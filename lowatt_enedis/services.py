...
ACCORD_CLIENT_OPTIONS = {
    ...
    '--denomination': {
        'help': "Dénomination sociale de la personne morale ayant donné son "
        "accord",
    },
    '--nom': {
...
ACCORD_CLIENT_EXTENDED_OPTIONS = {
    ...
    '--siret': {
        'help': "Numéro de SIRET de l'établissement principal de la personne "
        "morale",
    },
}


def _accord_client(client, args, extended=False,
    ...
    if ptype is not None:
        for option in unavailable_options:
            if getattr(args, option, None):
                raise ValueError(
                    "L'option {} ne peut être spécifiée que pour une personne "
                    "{}.".format(option, ptype.lower()),
                )

        who = create_from_options(client, args, 'Personne{}Type'.format(ptype),
                                  who_options)
        setattr(accord, 'personne{}'.format(ptype), who)
...
@register('search', 'RecherchePoint-v2.0', {
    '--siret': {
        'help': 'n° de siret',
    },
    '--compteur': {
...
def search_point(client, args):
    criteria = create_from_options(client, args, 'CriteresType', {
        'siret': 'numSiret',
    ...
    address = create_from_options(client, args, 'AdresseInstallationType', {
...
@register('technical', 'ConsultationDonneesTechniquesContractuelles-v1.0', {
    'prm': {
...
def point_technical_data(client, args):
    print(client.service.consulterDonneesTechniquesContractuelles(
        get_option(args, 'prm'), get_option(args, 'login'),
        _boolean(get_option(args, 'autorisation')),
    ))


@register('measures', 'ConsultationMesures-v1.1', {
    'prm': {
        'help': 'identifiant PRM du point',
    },
    'contrat': {
...
def point_measures(client, args):
    print(client.service.consulterMesures(
        get_option(args, 'prm'), get_option(args, 'login'),
        get_option(args, 'contrat'), _boolean(get_option(args, 'autorisation')),
    ))


@register('details', 'ConsultationMesuresDetaillees-v2.0', dict_from_dicts({
    'prm': {
        'help': 'identifiant PRM du point',
    },
    'type': {
...
def point_detailed_measures(client, args):
    demande = create_from_options(
        client, args, 'DemandeType', dict_from_dicts({
...
def measures_resp2py(resp):
    ...
    for point in resp.grandeur[0].mesure:
        data.append((point.d.astimezone(UTC), point.v))

    return data


@register(
    'cmdHisto', 'CommandeTransmissionHistoriqueMesures-v1.0', dict_from_dicts({
        'prm': {
            'help': 'identifiant PRM du point',
        },
        'contrat': {
    ...
    }, ACCORD_CLIENT_OPTIONS, ACCORD_CLIENT_EXTENDED_OPTIONS, MESURES_OPTIONS, {
...
def point_cmd_histo(client, args):
    ...
    demande.donneesGenerales = create_from_options(
        client, args, 'DemandeDonneesGeneralesType', {
    ...
    demande.historiqueMesures = create_from_options(
        client, args, 'DemandeHistoriqueMesuresType',
    ...
    demande.historiqueMesures.declarationAccordClient = _accord_client(
        client, args, True, 'ns0:DeclarationConsentementType',
    )
...
@register(
    'cmdInfraJ', 'CommandeTransmissionDonneesInfraJ-v1.0', dict_from_dicts({
        'prm': {
            'help': 'identifiant PRM du point',
        },
        'contrat': {
...
def point_cmd_infra_j(client, args):
    ...
    demande.donneesGenerales = create_from_options(
        client, args, 'DonneesGeneralesType', {
    ...
    demande.accesDonnees = create_from_options(
        client, args, 'DemandeAccesDonneesType', {
            'injection': 'injection',
            'soutirage': 'soutirage',
            'cdc': 'cdc',
            'idx': 'idx',
    ...
    demande.accesDonnees.declarationAccordClient = accord = _accord_client(
        client, args, xs_accord_type='ns1:DeclarationAccordClientType',
    )
